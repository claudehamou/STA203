...
On remarque que les courbes ROC des données *training* et *test* se superposent. Donc le modèle se comporte de la même manière sur les données de *test* que sur les données de *training*. **//// EST CE QUE C'EST VRAI CA ?????? ////**
...
Pour ne pas avoir à faire le calcul lors de la compilation du rapport, nous avons enregistrer les données.
...
La régression ridge est un type de régression pénalisée....
...
legend("bottomright", 
       legend=c("ROC Modèle contenant toutes les variables",
                "ROC Modèle contenant les variables selection en Partie 1"),
       col=c(3,2), lty=c(1,1))
```